...
drop table if exists manager_dim cascade;
...
drop table if exists deliver_dim cascade;
...
select row_number() over(), d.country, d.state, d.city, d.postal_code, d.region
from (select distinct o.country, o.state, o.city, o.postal_code, o.region
...
select row_number() over() + 100, s.ship_mode from (select distinct ship_mode from orders) as s;
...
from orders o 
	join product_dim p using(product_name, category, subcategory)
